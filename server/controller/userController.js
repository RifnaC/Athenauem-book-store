...
exports.isSuperAdmin = (req, res, next) => {
    const token = req.headers.authorization;
    if (!token){
        return res.status(403).send({message:"permission denied. No token provided."});
    }
    try{
        const decodeToken = jwt.verify(token.split(" ")[0], JWT_SECRET);
        if(!decodeToken.isSuperAdmin) {
            return res.status(403).send({message : "Permission denied!! You are not allowed to access this"})
        }
        next();
    }catch(error){
